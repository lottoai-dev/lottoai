...
export default function GenerateScreen() {
  ...
  const handleGenerate = () => {
    ...
    if (selectedGame.superStar) ss = generateNumbers(1, selectedGame.superStar.max)[0];
  ...
  const buildCoupon = (entry: { game: string; numbers: number[]; bonus: number[]; superStar?: number | null }, idOffset = 0) => ({
    ...
    bonus: entry.bonus,
    superStar: entry.superStar ?? undefined,
            ...
            <View style={s.segmentWrap}>
              {filterOptions.map((n) => {
                ...
                return (
                  <Pressable
                    key={n}
                    onPress={() => { setEvenCount(n); setBalanced(false); Haptics.selectionAsync(); }}
